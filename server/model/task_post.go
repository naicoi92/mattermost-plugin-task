...
// TaskPost kinds distinguish what role a tracked card post plays. The MVP uses
// Channel (the card in the task's channel) and DM (the card in the assignee's
// direct message); future kinds (e.g. a follower's DM) can be added without a
// schema change because kind is a free-form VARCHAR(32).
const (
...
// A task may have several (one in its channel, one in the assignee's DM, and
// later possibly more), each tagged with a kind so the card-update logic knows
// where to push refreshes.
type TaskPost struct {
	...
	// Kind is one of the PostKind* constants (channel/dm/future).

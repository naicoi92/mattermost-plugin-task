...
export default function UserPicker({
	...
	// an opaque value.
	const resolved =
		Boolean(value) && selectedLabel.length > 0 && selectedLabel !== value;
	...
	return (
		...
		>
			...
			{open && (
				<div className="user-picker__panel" role="listbox">
					<input
						...
						style={{
							border: 0,
							borderBottom: "1px solid var(--task-border)",
							borderRadius: 0,
						}}
					/>
					{loading && <div className="user-picker__loading">{"…"}</div>}
					{!loading && error && (
						<div className="user-picker__empty">{error}</div>
					)}

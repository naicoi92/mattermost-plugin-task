...
describe('task cache upsert', () => {
    ...
    test('UPSERT_TASK replaces an existing task', () => {
        const s1 = reducer(undefined, {
            type: ACTION_TYPES.UPSERT_TASK,
            task: task('1', {summary: 'a'}),
        });
        const s2 = reducer(s1, {
            type: ACTION_TYPES.UPSERT_TASK,
            task: task('1', {summary: 'b'}),
        });
        expect(summaryOf(s2.tasks['1'])).toBe('b');
    });

    test('UPSERT_TASK refreshes the selected task when it is the one updated', () => {
        const selected = reducer(undefined, {
            type: ACTION_TYPES.SELECT_TASK,
            taskID: '1',
            task: task('1', {summary: 'old'}),
        });
        const state = reducer(selected, {
            type: ACTION_TYPES.UPSERT_TASK,
            task: task('1', {summary: 'new'}),
        });
    ...
    test('UPSERT_TASK does not touch the selected task when updating another', () => {
        const selected = reducer(undefined, {
            type: ACTION_TYPES.SELECT_TASK,
            taskID: '1',
            task: task('1', {summary: 'mine'}),
        });
        const state = reducer(selected, {
            type: ACTION_TYPES.UPSERT_TASK,
            task: task('2', {summary: 'other'}),
        });
...
describe('stale-event drop (seq/updated_at, #32)', () => {
    test('UPSERT_TASK with an older seq is dropped', () => {
        ...
        const state = reducer(s1, {
            type: ACTION_TYPES.UPSERT_TASK,
            task: task('1', {summary: 'stale'}),
            seq: 50,
        });
    ...
    test('seq is tracked per task independently', () => {
        ...
        const state = reducer(s2, {
            type: ACTION_TYPES.UPSERT_TASK,
            task: task('1', {summary: 'upd'}),
            seq: 6,
        });
...
describe('COMMENT_REV_BUMP (Task 6 — comment refetch signal)', () => {
    ...
    test('a stale seq (<= current) is ignored — commentRev unchanged', () => {
        ...
        const stale = reducer(s1, {
            type: ACTION_TYPES.COMMENT_REV_BUMP,
            taskID: 'T',
            seq: 5,
        });
        expect(stale.commentRev.T).toBe(5);
        const older = reducer(s1, {
            type: ACTION_TYPES.COMMENT_REV_BUMP,
            taskID: 'T',
            seq: 4,
        });
